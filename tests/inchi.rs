use iupac::{graph::Graph, inchi::InChI, parser::parse};
use paste::paste;
...
test_inchi! {
    ...
    // Mobile hydrogens are not implemented
...
fn test_inchi_impl(iupac: &str, inchi: &str) {
    ...
    let inchi_graph = UnGraph::from(&inchi);

    assert!(is_isomorphic(&iupac_graph, &inchi_graph));
}

#[test]
fn test_parse_mobile_hydrogens_inchi() {
    for inchi in [
        ...
        let _inchi: InChI = inchi.parse().unwrap();
    }
}
